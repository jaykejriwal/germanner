    ...
    def __init__(
        ...
        if phase != Phase.Predict:
            self._y = tf.placeholder(
                tf.int32, shape=[batch_size, label_size])
        ...
        if phase == Phase.Train or Phase.Validation:
            input_mask = tf.sequence_mask(self._lens, maxlen = tf.shape(self._y)[1], dtype = tf.float32)
        ...
        if phase == Phase.Validation:
            # Predicted labels
            self._plabels = tf.cast(tf.argmax(logits, axis=2), tf.int32)
            
            # Correct labels
            correct_prediction = tf.equal(self.plabels, self._y)
			
			# Zero out correctness for inactive steps.
            correct_prediction = tf.multiply(input_mask, tf.cast(correct_prediction, tf.float32))

            # Compensate for inactive steps
            correct_prediction = tf.reshape(correct_prediction, [-1])
            correct_prediction = tf.truediv(correct_prediction, tf.reduce_mean(input_mask))
